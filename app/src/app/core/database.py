...
class DatabaseManager:
    ...
    def add_sequence_item(self, sequence_id: str, clip_id: str, position: int, notes: str = None) -> SequenceItem:
        with self.get_session() as session:
            ...
            existing_items = (
                ...
                .filter(SequenceItem.sequence_id == sequence_id, SequenceItem.position >= position)
                .order_by(SequenceItem.position.desc())
                .all()
            )
    ...
    def _apply_filters(self, query, node, session):
        ...
        from app.core.query_parser import ComparisonNode, InNode, LogicalNode
